...
describe('express(1)', function () {
  ...
  describe('default prompt answers', function () {
    var ctx = setupTestEnvironment(this.fullTitle())

    it('should create basic app', function (done) {
      runRaw(ctx.dir, ['deleteme'], function (err, code, stdout, stderr) {
    ...
    it('should have basic files', function () {
      assert.notStrictEqual(ctx.files.indexOf('server/bin/www.js'), -1)
      assert.notStrictEqual(ctx.files.indexOf('server/app.js'), -1)
      assert.notStrictEqual(ctx.files.indexOf('server/routes/index.js'), -1)
      assert.notStrictEqual(ctx.files.indexOf('server/routes/users.js'), -1)
      assert.notStrictEqual(ctx.files.indexOf('package.json'), -1)
      assert.notStrictEqual(ctx.files.indexOf('.env'), -1)
      assert.notStrictEqual(ctx.files.indexOf('.gitignore'), -1)
      assert.notStrictEqual(ctx.files.indexOf('.babelrc'), -1)
    })

    // TODO: add this test back after updating
    ...
    it('should export an express app from app.js', function () {
      var file = path.resolve(ctx.dir, 'server/app.js')
